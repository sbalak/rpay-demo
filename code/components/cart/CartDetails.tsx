...
export default function CartDetails() {
  ...
  const load = async () => {
    try {
      ...
      log.error("CartDetails.tsx/loadCartDetails(): Error loading cart details: " + error);
    }
  };
  ...
  const matchOffer = async (restaurantId: string) => {
    try {
      ...
      log.error("CartDetails.tsx/matchOffer(): Error matching offer: " + error);
    }
  };
  ...
  const loadLoyalty = async (restaurantId: string) => {
    try {
      ...
      log.error("CartDetails.tsx/loadLoyalty(): Error loading loyalty: " + error);
    }
  };
  ...
  const redeemReward = async (rewardId: string, restaurantId: string) => {
    try {
      ...
      log.error("CartDetails.tsx/redeemReward(): Error redeeming reward: " + error);
    }
  };
  ...
  const forfeitReward = async (voucherId: string, restaurantId: string) => {
    try {
      ...
      log.error("CartDetails.tsx/forfeitReward(): Error forfeiting reward: " + error);
    }
  };
  ...
  const processCheckout = async () => {
    try {
      ...
      log.error("CartDetails.tsx/processCheckout(): Error processing checkout: " + error);
    }
  };
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      log.error("CartDetails.tsx/checkout(): Error during checkout: " + error);
    }
    finally {
  ...
  const handleAddItem = async (restaurantId: string, foodId: string) => {
    try {
      ...
      log.error("CartDetails.tsx/addToCart(): Error adding item to cart: " + error);
    }
  };
  ...
  const handleRemoveItem = async (restaurantId: string, foodId: string) => {
    try {
      ...
      log.error("CartDetails.tsx/removeFromCart(): Error removing item from cart: " + error);
    }
  };
    ...
    const loadUser = async () => {
      if (authState.userId) {
        try {
          ...
          log.error("CartDetails.tsx/loadUserDetails(): Error loading user details: " + error);
        }
      }
    };
    loadUser();
