...
export default function StoreDetails() {
  ...
  const loadRestaurantDetails = async () => {
    try {
      ...
      log.error("store/[id].tsx/loadRestaurantDetails(): Error loading restaurant details: " + e);
    }
  };
  ...
  const loadRestaurantMenu = async (searchText: string) => {
    try {
      ...
      log.error("store/[id].tsx/loadMenu(): Error loading restaurant menu: " + e);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleAddItem = async (restaurantId: string, foodId: string) => {
    ...
    try {
      ...
      log.error("store/[id].tsx/addToCart(): Error adding item to cart: " + e);
    }
  };
  ...
  const handleRemoveItem = async (restaurantId: string, foodId: string) => {
    ...
    try {
      ...
      log.error("store/[id].tsx/removeFromCart(): Error removing item from cart: " + e);
    }
  };
  ...
  const handleCartReplaceAndAddItem = async () => {
    ...
    try {
      ...
      log.error("store/[id].tsx/replaceCart(): Error replacing cart: " + error);
    }
  };
